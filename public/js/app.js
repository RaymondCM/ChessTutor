...
function Init_Stockfish() {
    ...
    stockfishEngine.onmessage = function (event) {
        ...
        if ((game_playerSide === side) && t_enable)
            gameTutor.onEngine(move.substr(0, 2), move.substr(2, 4));
...
function tutor(side, boardPosition) {
    ...
    this.side = side
        , this.bestMove = null
        , this.undevelopedPieces = null
        , this.undevelopedPiecesCount = null
        , this.castlePossible = true
        , this.playerMoves = []
        , this.opponentMoves = []
        , this.turnCount = 0
    ...
    this.calcUndeveloped = function (boardPosition) {
        ...
        for (var property in boardPosition)
            if (startPosition.hasOwnProperty(property) &&
                (boardPosition[property].substr(0, 1) === this.side) &&
    ...
    this.bestTopic = function () {
        ...
        if ((this.turnCount > 2) && (tutorKnowledge.turnCount < 9))
