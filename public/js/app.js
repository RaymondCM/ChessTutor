$(document).ready(function () {
    ...
    */
    ...
    socket_roomID = null;
    if (t_enable) gameTutor = new tutor(game_playerSide);
    Init_Stockfish();
    Init_Chessboard();
...
function Init_Chessboard() {
    ...
    if (!ct_debug) $('#Debug').css('display', 'none');
    board = "";
    game = new Chess();
    // do not pick up pieces if the game is over
    // only pick up pieces for the side to move
    var onDragStart = function (source, piece, position, orientation) {
        if (game.game_over() === true || (game.turn() === 'w' && piece.search(/^b/) !== -1) || (game.turn() === 'b' && piece.search(/^w/) !== -1) || game_aiMode === 1 || game.turn() !== game_playerSide) {
            return false;
        }
    };
    ...
    gameTutor.calcUndeveloped(board.position());
...
function removeHighlighting() {
    ...
    if (cb_permHighlighted.length !== 0) board.highlightSquare(cb_permHighlighted.squares, true, cb_permHighlighted.colours);
};

function highlightSquare(square, perm, colour) {
    if (typeof colour === 'undefined') colour = [cb_boardTheme[2], cb_boardTheme[3]];
...
function MovePiece(from, to) {
    ...
    board.position(game.fen(), false);
    t_enable && gameTutor.moveMade(from, to, game.turn(), turnCount);
    t_enable && gameTutor.calcUndeveloped(board.position());
    turnCount++;
    gameTutor.turnCount = turnCount;
    updateStatus();
}

function updateStatus() {
    if (game_aiMode == 2) game_playerSide = (game_playerSide == 'w' ? 'b' : 'w');
    checkForTaken(board.position());
    if (ct_debug) updateDebugLog();
    if (fenHistory.lengh <= cb_fenHistoryMaxLength) fenHistory.push(game.fen());
    else {
        fenHistory.shift();
...
function updateScale(depth) {
    var doc = document
        ...
        , rawRange = [bScore, wScore], //Get Min and Max Values From Array (Use 0 instead of Math because 3 bytes cheaper)
...
function checkForTaken(boardPosition) {
    ...
    for (var property in boardPosition)
        if (boardPosition.hasOwnProperty(property)) p[boardPosition[property]]++;
    ...
    for (var property in p) drawImg(cb_pieceTheme(property), (property.charAt(0) == 'w') ? capturedWhite : capturedBlack, p[property]);
}
...
function Init_Stockfish() {
    if (sf_accurateCentipawns) stockfishEngine.postMessage("setoption name MultiPV value 50");
    stockfishEngine.onmessage = function (event) {
        if (event.data.toString().substr(0, 8) !== 'bestmove') {
            if (event.data.toString().substr(0, 10) == 'info depth') lastPonder = event.data;
            return;
        }
        getScore(lastPonder);
        var side = game.turn()
        ...
        if ((game_playerSide === side) && t_enable) gameTutor.onEngine(move.substr(0, 2), move.substr(2, 4));
...
function getScore(a) {
    infoToObj = function (a) {
        ...
        for (var e = 0, d = ""; e < b.length; e++) {
            ...
            else d = (d == "" ? b[e] : d + '_' + b[e]);
        }
        return c;
    };
    var msgObj = infoToObj(a);
    if (game.turn() === 'w' && msgObj.hasOwnProperty("score_cp")) sf_scoreWhite = msgObj.score_cp;
    else if (msgObj.hasOwnProperty("score_cp")) sf_scoreBlack = msgObj.score_cp;
    updateScale();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
function tutor(side, boardPosition) {
    //ATTRIBUTES
    this.side = side, this.bestMove = null, this.undevelopedPieces = null, this.undevelopedPiecesCount = null, this.castlePossible = true, this.playerMoves = [], this.opponentMoves = [], this.turnCount = 0
        //PUBLIC METHODS
    this.calcUndeveloped = function (boardPosition) {
        var undevelopedPieces = {};
        for (var property in boardPosition)
            if (startPosition.hasOwnProperty(property) && (boardPosition[property].substr(0, 1) === this.side) && (boardPosition[property] === startPosition[property])) undevelopedPieces[property] = startPosition[property];
        this.undevelopedPieces = undevelopedPieces;
        this.undevelopedPiecesCount = Object.keys(undevelopedPieces).length;
    };
    this.moveMade = function (from, to, side, turnCount) {
        (side === game_playerSide) ? this.playerMoves.push({
            from: from
            , to: to
    ...
    this.onEngine = function (from, to) {
        pushMessage("Engine says: " + from + " " + to);
        this.bestMove = {
            from: from
            , to: to
        };
    ...
    this.bestTopic = function () {
            ...
            if ((this.turnCount > 2) && (tutorKnowledge.turnCount < 9)) weights.turnCount = 10;
            else weights.turnCount = tutorKnowledge.turnCount;
        }
        //PRIVATE METHODS
